...
func TestVerifyObjectStore(t *testing.T) {
	...
	const head = "aa1c7cbe7b72b787270f456d61fe0046a46cba12"

	openMeta := func(t *testing.T) *meta.Metadata {
		t.Helper()
		m, err := meta.Open(ctx, meta.Config{LocalPath: filepath.Join(t.TempDir(), "meta.sqlite3")})
		if err != nil {
			t.Fatalf("meta.Open: %v", err)
	...
	withRef := func(t *testing.T) *meta.Metadata {
		t.Helper()
		m := openMeta(t)
		r, err := m.CreateRepo(ctx, repoName, "master")
		if err != nil {
			t.Fatalf("CreateRepo: %v", err)
		}
		if err := m.CASRef(ctx, r.ID, "refs/heads/master", meta.ZeroSHA, head); err != nil {
			t.Fatalf("CASRef: %v", err)
		}
		return m
	}

	t.Run("fresh instance with no refs passes", func(t *testing.T) {
		m := openMeta(t)
		if _, err := m.CreateRepo(ctx, repoName, "master"); err != nil {
			t.Fatalf("CreateRepo: %v", err)
		}
		// A repo exists but nothing was pushed — an empty store is correct.
		if err := verifyObjectStore(ctx, m, store.NewMem()); err != nil {
			t.Errorf("fresh instance rejected: %v", err)
		}
	})

	t.Run("refs with objects present passes", func(t *testing.T) {
		m := withRef(t)
		s := store.NewMem()
		if err := s.Put(ctx, repoName+"/objects/aa/1c7cbe", bytes.NewReader([]byte("x"))); err != nil {
			t.Fatalf("Put: %v", err)
		}
		if err := verifyObjectStore(ctx, m, s); err != nil {
			t.Errorf("healthy store rejected: %v", err)
		}
	})

	t.Run("refs without objects fails loud", func(t *testing.T) {
		m := withRef(t)
		// Refs restored but the store is empty at the prefix — the regression.
		err := verifyObjectStore(ctx, m, store.NewMem())
		if err == nil {
